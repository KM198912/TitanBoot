...
# Create disk image (boot + loader + kernel)
$(OS_IMG): $(BOOT_BIN) $(LOADER_BIN) $(KERNEL_BIN)
	...
	# Write kernel (starting at sector 18)
...
# Run in QEMU
run: $(OS_IMG)
	qemu-system-i386 -drive file=$(OS_IMG),format=raw -debugcon stdio -m 4G -enable-kvm -cpu host
